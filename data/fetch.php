...
foreach($result as $row)
{
	...
	if($row["image"] != '')
	{
		$image = '<img src="./public/images/avatar/ ' .$row["image"].'" class="img-thumbnail" width="50" height="35" />';
	}
	else
	{
		$image = '';
	...
	$sub_array[] = $row["Score"];
	$sub_array[] = $row["actif"];
	$sub_array[] = '<button type="button" name="update" id="'.$row["id"].'" class="btn btn-primary btn-xs update">Modifier</button>';
